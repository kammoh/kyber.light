...
int main(void)
{
  ...
  if(memcmp(ka,kb,KYBER_SYMBYTES)){
    fprintf(stderr, "Error in UAKE\n");
    for(int i=0; i < KYBER_SYMBYTES; i+=1){
        if(ka[i] != kb[i]){
            fprintf(stderr, "i: %d ka[i]=%d kb[i]=%d\n",i,ka[i], kb[i]);
        }
    }
    exit(1);
  }
  ...
  if(!memcmp(ka,zero,KYBER_SYMBYTES)){
    fprintf(stderr, "Error: UAKE produces zero key\n");
    exit(1);
  }
  ...
  if(memcmp(ka,kb,KYBER_SYMBYTES)){
    fprintf(stderr, "Error in AKE\n");
    exit(1);
  }
  
  if(!memcmp(ka,zero,KYBER_SYMBYTES)){
    fprintf(stderr, "Error: AKE produces zero key\n");
    exit(1);
  }
