 ...
 *(secret key)
 **************************************************/
static void pack_sk(unsigned char *r, polyvec *sk) { polyvec_tobytes(r, sk); }
 ...
 *(uniform mod q)
 *              - const unsigned char *buf: pointer to input buffer (assumed to
 *be uniform random bytes)
                ...
                int transposed) // Not static for benchmarking
{
  ...
  const unsigned int maxnblocks =
      (530 + XOF_BLOCKBYTES) /
      XOF_BLOCKBYTES; /* 530 is expected number of required bytes */
...
void unpack_pk_at_nontt(polyvec *pkpv_p, polyvec *at, const unsigned char *pk) {
  ...
  polyvec_invntt(pkpv_p);
  polyvec_montgomery_reduce(pkpv_p);
  ...
  for (int i = 0; i < KYBER_K; i++) {
    polyvec_invntt(&at[i]);
    polyvec_montgomery_reduce(&at[i]);
    polyvec_reduce(&at[i]);
  }
}
