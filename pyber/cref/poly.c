...
void poly_freeze(poly *a){
    for (int i = 0; i < KYBER_N; i++) {
        ...
        if (a->coeffs[i] > KYBER_Q - 1) {
            fprintf(stderr, "***WARNING***: a->coeffs[i] (%d) > KYBER_Q - 1 \n", a->coeffs[i] > KYBER_Q - 1);
            a->coeffs[i] = a->coeffs[i] % KYBER_Q;
        }
    }
}
...
void poly_getnoise_bytes(unsigned char *r, const unsigned char *seed,
                         unsigned char nonce) {
  unsigned char extseed[KYBER_SYMBYTES + 1];
  int i;
  ...
  shake256(r, KYBER_ETA * KYBER_N / 4, extseed, KYBER_SYMBYTES + 1);
}
...
void poly_tomsg_nofreeze(unsigned char msg[KYBER_SYMBYTES], const poly *a) {
  uint16_t t, t2;
  int i, j;
  ...
  for (i = 0; i < KYBER_SYMBYTES; i++) {
    ...
    for (j = 0; j < 8; j++) {
      ...
      t2 = ((((KYBER_Q - a->coeffs[8 * i + j]) << 1) + KYBER_Q / 2) / KYBER_Q) & 1;
      if (t != t2){
          fprintf(stderr, "t(%d) != t2(%d)\n", t, t2);
          exit(1);
      }
      msg[i] |= t << j;
    }
