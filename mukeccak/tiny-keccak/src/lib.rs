...
impl Keccak {
    ...
    pub fn theta(&mut self) -> PyResult<()> {
        ...
        for col in 0..5 {
            for row in 0..5 {;
                self.state[Self::ij_to_linear(row, col)] ^= parity[(col + 4) % 5] ^ parity[(col + 1) % 5].rotate_left(1);
            }
        }
        Ok(())
    }
    ...
    pub fn pi(&mut self) -> PyResult<()> {
        ...
        for row in 0..5 {
            for col in 0..5 {
                self.state[Self::ij_to_linear(row, col)] = state[Self::ij_to_linear(col, 3 * row + col)];
            }
        }
        Ok(())
    }
    ...
    pub fn dump_state(&self, msg: &str) -> PyResult<()> {
        println!("{}", msg);
    ...
    pub fn keccakf(&mut self, absorb: bool) -> PyResult<()> {
        if absorb {
            self.xorin()?;
        }
        ...
        loop {
            ...
            if round_cntr == NUM_ROUNDS {break}
            self.rho()?;
            round_cntr += 1;
        }
